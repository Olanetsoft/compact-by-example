...
Check for overflow before performing arithmetic to prevent exceeding type bounds.
...
**Example**: Calculating percentages, fees

```compact
// Calculate 1% fee
const fee = (amount * 1) / 100; // Safe because 1/100 < 1
```
...
```compact
// Increase balance (minting, receiving)
const currentBalance = getBalance(to);
assert(currentBalance <= MAX_UINT128 - amount, "Balance overflow");
balances.insert(disclose(to), disclose(currentBalance + amount));

// Decrease balance (burning, sending)
const currentBalance = getBalance(from);
assert(currentBalance >= amount, "Insufficient balance");
balances.insert(disclose(from), disclose(currentBalance - amount));
```
...
assert(totalSupply >= amount, "Burn exceeds supply");
totalSupply = disclose(totalSupply - amount);
```
...
balance = balance + largeAmount; // Could wrap around to 0!
```

If `balance = MAX_UINT128` and you add 1, without checks it wraps to 0, destroying value.

**With checks**:

```compact
// Safe: Transaction fails before corruption
assert(balance <= MAX_UINT128 - largeAmount, "Overflow");
balance = balance + largeAmount; // Only executes if safe
```

The transaction reverts instead of corrupting state.
